...
class loginControl extends BaseHomeControl {
	...
	public function usersaveOp() {
		...
		if($_POST['mobile'] != $_SESSION['mobile'] || $_POST['sms'] != $_SESSION['mobile_code']){
			showDialog($lang['login_usersave_wrong_sms']);
		}
	...
	public function find_passwordOp(){
		...
		$extend_js = "<script src='".SHOP_SITE_URL."/index.php?act=login&op=send_email&exeid=".$member['member_id']."'></script>";
		echo $extend_js;
		//showMessage($lang['login_password_email_success'].$extend_js,SHOP_SITE_URL);
		//showDialog($lang['login_password_email_success'],SHOP_SITE_URL);
		//showDialog($lang['login_password_email_success'], SHOP_SITE_URL, 'succ', 'CUR_DIALOG.close();');
		echo '<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />';
		echo "<script>alert('邮件发送成功，请注意查收！'); top.location='".SHOP_SITE_URL."';</script>";
	}

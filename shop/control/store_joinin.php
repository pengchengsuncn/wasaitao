...
class store_joininControl extends BaseHomeControl {
    ...
    public function step3Op() {
        if(!empty($_POST)) {
           ...
           /* $param['bank_account_name'] = $_POST['bank_account_name'];
            ...
            $param['id_cart_number_electronic'] = $this->upload_image('id_cart_number_electronic');
            $param['id_cart_number_electronic1'] = $this->upload_image('id_cart_number_electronic1');
            ...
            $model_store_joinin = Model('store_joinin');
            $model_store_joinin->modify($param, array('member_id'=>$_SESSION['member_id']));
           // Model()->execute('UPDATE `wst_store_joinin` SET business_licence_number= $param["business_licence_number"] WHERE member_id = '.$_SESSION['member_id']);
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['business_sphere'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"500","message"=>"法定经营范围不能为空且必须小于50个字"),
            array("input"=>$param['business_licence_number_electronic'], "require"=>"true","message"=>"营业执照电子版不能为空"),
            array("input"=>$param['id_cart_number_electronic'], "require"=>"true","message"=>"身份证正面电子版不能为空"),
            array("input"=>$param['id_cart_number_electronic1'], "require"=>"true","message"=>"身份证反面电子版不能为空"),
        );
        $error = $obj_validate->validate();
        if ($error != ''){
            showMessage($error);

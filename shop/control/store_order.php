...
class store_orderControl extends BaseSellerControl {
	...
	public function indexOp() {
        ...
        foreach ($order_list as $key => $order_info) {
        	...
        	//显示调整费用
        	$order_list[$key]['if_modify_order_price'] = $model_order->getOrderOperateState('modify_order_price',$order_info);
	...
	public function change_stateOp() {
		...
		try {
    		...
    		*/
    		...
    		elseif ($state_type == 'modify_order_price') {
    		    $this->_change_state_modify_order_price($order_info);
    		    $message = Language::get('store_order_edit_price_success');
    		}
	 ...
	 * @param unknown $order_info
	 */
	private function _change_state_modify_order_price($order_info) {
	    $order_id = $order_info['order_id'];
	    $model_order = Model('order');
	    if(chksubmit()) {
	        /*$if_allow = $model_order->getOrderOperateState('modify_order_price',$order_info);
	        if (!$if_allow) {
	            throw new Exception(L('invalid_request'));
	        }*/
	        $data = array();
	        ...
	        $data['shipping_fee'] = abs(floatval($_POST['shipping_fee']));
	        $data['goods_amount'] = array('exp','order_amount-'.$data['shipping_fee']);
	        $update = $model_order->editOrder($data,array('order_id'=>$order_id));
	        if (!$update) {
	            throw new Exception(L('nc_common_save_fail'));
	        }
	        ...
	        $data['log_role'] = 'seller';
			$data['log_user'] = $_SESSION['member_name'];
	        $data['log_msg'] = L('order_log_edit_price');
	        $model_order->addOrderLog($data);
	    } else {
	        Tpl::output('order_id',$order_id);
	        Tpl::showpage('store_order.edit_price_order','null_layout');
	        exit();
	    }
	}
